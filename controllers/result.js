...
exports.createComment = async (req, res) => {
    if (
        (await req.app
            .get('database')
            .collection('users')
            .findOne({ _id: new ObjectId(req.session.userid) })) == null
    ) {
        res.redirect('/login')
        return
    }
...
exports.loadResults = async (req, res) => {
    try {
        ...
        let resultPet = await req.app
            ...
            .collection('pets')
            .find({
                //https://stackoverflow.com/a/25670767
                $and: [{ species: req.body.species }, { age: req.body.age }]
            })
            .toArray()
